...
interface FacilityItemsManagerProps {
  value: FacilityItem[]
  onChange: (items: FacilityItem[]) => void
}

export function FacilityItemsManager({ value, onChange }: FacilityItemsManagerProps) {
  ...
  const updateItem = (id: string, field: keyof FacilityItem, fieldValue: string) => {
    ...
    <div className="space-y-4">
      ...
      {value.length === 0 ? (
        <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center">
          <p className="text-gray-500 mb-4">Belum ada facility item. Klik "Tambah Fasilitas" untuk menambahkan.</p>
        </div>
      ) : (
                ...
                <div className="bg-gray-50 px-4 py-3 flex items-center justify-between">
                  <div className="flex items-center space-x-3 flex-1">
                    ...
                    <span className="text-sm font-medium text-gray-700">
                      {item.caption || `Fasilitas ${index + 1}`}
                    </span>
                  </div>
                  ...
                  <div className="p-4 space-y-4 bg-white">
                    <div>
                      ...
                      <input
                        ...
                        className="hidden"
                        id={`facility-image-${item.id}`}
                      />
                      <label
                        htmlFor={`facility-image-${item.id}`}
